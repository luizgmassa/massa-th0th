...
// ── P6-SEARCH-01: searchability integration (singleton reset to temp DB) ─────
//
// This block mirrors bootstrap-service.test.ts P4-SEARCH-01: reset the
// MemoryRepository singleton to a temp dataDir, run the REAL insert +
// fullTextSearch path to prove the dual-write handoff memory is
// FTS-searchable. Self-contained; restores the singleton in afterEach.

import { MemoryRepository } from "../data/memory/memory-repository.js";

describe("HandoffService — P6-SEARCH-01 dual-write memory searchability (integration)", () => {
  ...
  it("the dual-write memory is findable via MemoryRepository.fullTextSearch", async () => {
    // Use the real MemoryRepository as the dual-write target.
    const token = "uniquehandoffsearchtoken";
    const svc = new HandoffService({
      store: makeFakeStore(),
      memoryRepo: {
        insert(input: InsertMemoryInput): void {
    ...
    const res = await svc.begin({
      projectId: "proj-handoff-search",
      summary: `Handoff contains ${token} for FTS discovery.`,
    });
    expect(res.ok).toBe(true);
    ...
    const found = hits.find((h) => h.id === res.memoryId);
