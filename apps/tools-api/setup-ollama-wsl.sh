...
if [ -f "$ENV_FILE" ]; then
    ...
    cat > "$ENV_FILE" << EOF
# massa-ai - Auto-generated by setup-ollama-wsl.sh
...
if curl -s "${OLLAMA_URL}/api/embeddings" \
    -d '{"model": "qwen3-embedding:8b", "prompt": "test"}' > /dev/null 2>&1; then
    echo "Embeddings working."
else
    echo "Could not reach embedding endpoint. Make sure qwen3-embedding:8b is pulled:"
