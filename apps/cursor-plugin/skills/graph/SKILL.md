...
allowed-tools: ["mcp__massa-th0th__get_references", "mcp__massa-th0th__go_to_definition", "mcp__massa-th0th__list_projects"]
---
...
2. Call `mcp__massa-th0th__go_to_definition` with `symbolName=$ARGUMENTS` to locate the definition first.
3. Call `mcp__massa-th0th__get_references` with the same `symbolName` (and `fqn` if provided by the definition lookup).
