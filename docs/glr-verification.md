...
massa-ai's grammars because:

1. **Grammars used by massa-ai** (TypeScript, JavaScript, Python, Go, Rust,
...
2. **Input sources** in massa-ai are source files from real projects. These
...
3. **The cap is a C-runtime safety valve**, not a JS-level configuration. Even
   ...
   degradation) rather than crashing. massa-ai's parser-readiness check
...
- **Affects massa-ai's grammars**: no
- **Defect found**: no
- **Fix needed**: no
...
callers, and does not affect massa-ai's grammar parsing in practice.