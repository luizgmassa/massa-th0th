...
2. Restore context before planning:
   - `th0th_recall` for exact-session continuity, durable decisions, rejected approaches, patterns, blockers, and handoffs.
   ...
   - `th0th_search` only counts as codebase evidence when it is fresh for the current repository path and commit/worktree state, or when the result is confirmed against source files read in this session.
...
- `th0th_remember` remains canonical for durable cross-session decisions, rejected approaches, reusable patterns, and verification recipes.
