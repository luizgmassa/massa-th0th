...
When verifying codebase claims, prefer the th0th tool chain FIRST — `th0th_list_projects`, `th0th_search`, `th0th_project_map`, `th0th_optimized_context` — before falling back to ast-grep / ripgrep / grep. Apply freshness and source-precedence rules: current source code overrides a stale index, and a stale index or durable memory never overrides current evidence. When uncertain about index freshness, read the live file directly.
