...
- [ ] `references/mcp-tools.md` is updated to cover all 52+ tools (not just the ~20 it currently documents) so workflows can reference it as the single tool-contract source.
...
| Refactoring `massa-ai-memory` or `synapse-usage` meta-skills | Those are separate skills; this feature edits `massa-ai` workflows + `mcp-tools.md` reference + `SKILL.md` router. |
...
| --- | --- | --- | --- |
| Canonical tool names = no `th0th_` prefix | Replace `recall` → `recall`, `search` → `search`, etc. in all workflows and references | `tool-definitions.ts` CANONICAL_ORDER and the OpenCode plugin expose un-prefixed names; `th0th_` prefix is a stale artifact of the th0th-era naming. The `massa-ai-memory` SKILL.md already uses un-prefixed names. | y (assumption) |
| `references/mcp-tools.md` is the single tool-contract reference | Expand its MCP Capability Matrix to cover all 52+ tools, keep the file name for backward-compat | Workflows already point to it; renaming would break validator anchors. | y (assumption) |
...
**User Story**: As a massa-ai workflow consumer, I want workflows to reference system tools by their canonical un-prefixed names so that the references match what the MCP server and OpenCode plugin actually expose.

**Why P1**: Every other story depends on consistent naming; the `th0th_` prefix is a stale artifact that diverges from `tool-definitions.ts` and the `massa-ai-memory` meta-skill.

**Acceptance Criteria**:

1. WHEN any file under `skills/massa-ai/` (workflows, references, SKILL.md) is inspected THEN no `th0th_`-prefixed tool name SHALL remain; all tool references SHALL use the canonical un-prefixed names from `tool-definitions.ts` CANONICAL_ORDER.
2. WHEN `references/mcp-tools.md` is inspected THEN its MCP Capability Matrix SHALL list all tools in `tool-definitions.ts` CANONICAL_ORDER (52+ tools), each with a one-line "Primary use" note.
...
**Independent Test**: `rg 'th0th_' skills/massa-ai/` returns zero matches across the full skill tree (workflows, references, SKILL.md).
...
**Independent Test**: `rg 'create_checkpoint|list_checkpoints|restore_checkpoint' skills/massa-ai/workflows/` returns matches in spec-driven.md, long-session.md, and restart-save.md.
...
**Independent Test**: `rg 'handoff_begin|handoff_accept|handoff_list_pending|handoff_cancel' skills/massa-ai/workflows/` returns matches in agent-handoff.md and restart-load.md.
...
**Why P1**: `bootstrap` is a shipped idempotent tool designed exactly for this; the onboarding workflow currently does manual `index` + `th0th_remember` but never calls `bootstrap`.
...
**Independent Test**: `rg 'bootstrap' skills/massa-ai/workflows/onboarding.md` returns a match.
...
**Independent Test**: `rg 'compact_snapshot' skills/massa-ai/workflows/long-session.md` returns a match.
...
**Independent Test**: `rg 'trace_path' skills/massa-ai/workflows/debug.md` and `rg 'impact_analysis' skills/massa-ai/workflows/architecture/ skills/massa-ai/workflows/refactor.md` return matches.
...
**Independent Test**: `rg 'execute_file|execute|batch_execute' skills/massa-ai/workflows/debug.md skills/massa-ai/workflows/general.md` return matches.
...
**Independent Test**: `rg 'synapse_task_begin|synapse_task_end|synapse_prefetch' skills/massa-ai/workflows/` return matches in spec-driven.md and feature.md.
...
**Why P2**: `read_file` and `symbol_snippet` are shipped massa-ai tools that provide symbol metadata and line-range snippets; the `massa-ai-memory` meta-skill lists `read_file` as priority 14 but the workflows never reference it.

**Acceptance Criteria**:

23. WHEN any workflow needs to read a file with symbol metadata THEN it SHALL prefer `read_file` over native Read, per the `massa-ai-memory` priority rule.
...
**Independent Test**: `rg 'read_file|symbol_snippet' skills/massa-ai/workflows/` return matches.
...
**Independent Test**: `rg 'memory_update|memory_delete|analytics' skills/massa-ai/workflows/` return matches.
...
**Independent Test**: `rg 'th0th_' skills/massa-ai/SKILL.md` returns zero; `rg 'checkpoint|handoff_begin|bootstrap|compact_snapshot|trace_path|impact_analysis|execute_file|synapse_prefetch|read_file|symbol_snippet' skills/massa-ai/SKILL.md` returns matches.
...
- [ ] Zero `th0th_`-prefixed tool names remain in `skills/massa-ai/` (workflows, references, SKILL.md).
- [ ] `references/mcp-tools.md` MCP Capability Matrix lists all 52+ tools from `tool-definitions.ts` CANONICAL_ORDER.
