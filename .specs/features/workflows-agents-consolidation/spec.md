...
The massa-th0th sub-agent skills (12 specialists) and the massa-th0th workflow skill (router + 39 workflows + ~30 references) currently live in two separate repositories. The 12 agent charters sit in the product repo (cwd) at `skills/<name>/`, while the workflows that should invoke those agents live in the separate `Useful-Agent-Skills` repo at `skills/massa-th0th/workflows/`. The workflows still use the pre-consolidation role names (`implementer`, `verifier`, `domain-mapper`, `coupling-auditor`, `deepening-architect`) and carry large blocks of duplicated inline prompt text (scope resolution, false-positive pass, severity rules, "use subagents only when useful") that now duplicate capability already captured in the 12 agent charters. This split makes the agents hard to discover from the workflows, keeps two sources of truth for dispatch contracts, and blocks the planned follow-up (noted in `skills/AGENTS.md:99`) to "update massa-th0th workflows to replace duplicated inline prompt sections with agent invocations."

## Goals

- [ ] Consolidate the massa-th0th workflow skill into the product repo so agents and workflows ship from one tree.
...
| Deleting the massa-th0th skill from Useful-Agent-Skills | That repo's lifecycle is the owner's decision; this feature copies the skill into the product repo and leaves the source untouched unless the user asks to remove it. |
...
| --- | --- | --- | --- |
| Destination repo = product repo (cwd) | Move both agents and the massa-th0th skill into the product repo | User answered "massa-th0th product repo (cwd)" in the clarifying question | y |
...
| Workflow skill path = `skills/massa-th0th/` | Place the full skill (router + workflows + references + scripts) at `skills/massa-th0th/` | User said "Move the entire new massa-th0th workflows into this repository, at skills/" | y |
...
| `massa-th0th-memory` and `synapse-usage` stay at `skills/` top level | They are meta-skills, not agent charters; the generator already excludes them | Generator `SPECIALIST_NAMES` array excludes them; moving them would break the meta-skill symlink references | y (assumption) |
...
**User Story**: As a massa-th0th maintainer, I want the 12 agent charters and the massa-th0th workflow skill to live in one repository tree so that I can edit agents and workflows without switching repos and so shipped plugins stay drift-free.
...
2. WHEN the product repo is inspected THEN `skills/massa-th0th/` SHALL contain `SKILL.md`, `workflows/`, `references/`, and `scripts/lessons.py` copied from the Useful-Agent-Skills `skills/massa-th0th/` tree.
...
**User Story**: As a massa-th0th user, I want the rewritten workflows to route, recall, persist, and gate exactly as before so that no workflow behavior changes.
...
13. WHEN any rewritten workflow is diffed against its pre-rewrite version THEN the routing header, `workflowSessionId` rule, reference-load list, `th0th_recall` step, persistence tags, and Evidence Gate step SHALL be unchanged in meaning (prose may be tightened but contracts preserved).
...
- WHEN `skills/massa-th0th/scripts/lessons.py` is copied THEN its internal relative paths (it references `skills/massa-th0th/...`) SHALL still resolve because the skill moves as a unit.
- WHEN a workflow references `references/agent-orchestration.md` THEN the path SHALL still resolve relative to `skills/massa-th0th/` after the move.
...
- [ ] The product repo contains `skills/agents/` (12 charters) and `skills/massa-th0th/` (full workflow skill) with no agents left at `skills/<name>/` top level (except `massa-th0th-memory` and `synapse-usage` meta-skills).
