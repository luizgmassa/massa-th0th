...
1. **Destination repo** → massa-th0th product repo (cwd). Move both agents and the workflow skill into the cwd. Useful-Agent-Skills keeps its copy; the user decides its lifecycle separately.
...
- **Product repo (cwd)** `/Users/luizmassa/Personal Projects/massa-th0th/`:
  - `skills/<name>/SKILL.md` for 12 specialists + 2 meta-skills (`massa-th0th-memory`, `synapse-usage`)
...
- **Useful-Agent-Skills** `/Users/luizmassa/Personal Projects/Useful-Agent-Skills/`:
  - `skills/massa-th0th/` = `SKILL.md` (router) + `workflows/` (39 files, ~2759 LOC) + `references/` (~30 files) + `scripts/lessons.py`
...
- AD-WAC-001: Destination = product repo (cwd). Agents → `skills/agents/`; workflow skill → `skills/massa-th0th/`.
...
- AD-WAC-004: `massa-th0th-memory` and `synapse-usage` stay at `skills/` top level (meta-skills, excluded by generator).
