...
massa-ai ships first-class plugin packages for Claude Code (`apps/claude-plugin/`, script-copy bundle) and OpenCode (`apps/opencode-plugin/`, npm package), but has **no plugin packages for Codex or Cursor**. Both tools currently receive only MCP server registration via `scripts/install-agents.ts` and a printed (never auto-written) hooks config guide via the root `install.sh:print_hooks_guide`. Web research (2026-07-23) confirmed both tools now have real native plugin systems that massa-ai is not using: Codex has a `/plugins` command + `.codex-plugin/plugin.json` manifest + `skills/` + `.mcp.json` + `hooks/`, and Cursor has `vscode.cursor.plugins.registerPath` + `vscode.cursor.mcp.registerServer` + a `hooks.json` event system covering 18+ events (including `sessionStart` and `preCompact`, which the codebase's "Cursor only has 3 events" claim marks as out of date).
...
| Native npm VS Code extension for Cursor via `@cursor/sdk` | Out of scope: massa-ai ships a hook-script + config bundle, not a compiled VS Code extension. `vscode.cursor.plugins.registerPath` auto-discovers a directory layout, so we ship a discoverable directory, not a compiled `.vsix`. |
...
| Hook script reuse | Reuse the single `massa-ai-hook.ts` Bun binary from `apps/claude-plugin/hooks/` (platform-neutral) as the shared emitter. Add Codex/Cursor-specific wrapper scripts or config that point at the binary. | Wave 6 N30 made the binary platform-neutral (`install.sh:486-488` "only the config wrapper differs"). Avoids duplicating 7 shell scripts. | y (codebase) |
...
| Subagent | Codex: no native subagent concept in the plugin manifest; skip. Cursor: supports `agents/` in a registered plugin path; ship `massa-ai-navigator.md` (reuse claude-plugin's). | Codex plugin manifest has no `agents/` field per docs. Cursor auto-discovers `agents/` per `registerPath` docs. | y (web research) |
...
**User Story**: As a Codex CLI user, I want a `massa-ai` plugin I can install via `codex plugin` or manual copy, so that I get massa-ai slash commands, MCP tools, and passive lifecycle capture inside Codex.

**Why P1**: Codex is one of the two tools the user explicitly named. It currently has no plugin package at all.

**Acceptance Criteria**:

1. WHEN a user runs `apps/codex-plugin/install.sh --user` THEN the system SHALL create `~/.codex/plugins/massa-ai/` containing `.codex-plugin/plugin.json`, `skills/*.md`, `hooks/hooks.json`, `.mcp.json`, and the `massa-ai-hook` binary (or a symlink to the repo copy), with an ownership marker on the plugin manifest
2. WHEN a user runs `apps/codex-plugin/install.sh --project` THEN the system SHALL create `./.codex/plugins/massa-ai/` with the same structure under the current project
...
4. WHEN the Codex plugin is installed AND `.mcp.json` is present THEN Codex SHALL auto-discover the massa-ai MCP server (command `npx @massa-ai/mcp-client` with `MASSA_AI_API_URL` env) without a separate `install-agents.ts` run for that scope. The installer SHALL print a hint: "If you also run `install-agents.ts --agent codex`, skip MCP — the plugin already registers it."
5. WHEN Codex hooks are trusted (via `/hooks`) AND a Codex lifecycle hook fires (any of the 6 wired events) THEN the `massa-ai-hook` binary SHALL POST to `/api/v1/hook` (single) or `/api/v1/hook/batch` with a 2s timeout, always exit 0, and never block the agent. The installer SHALL print a blocking warning after install: "Run `/hooks` in Codex to trust massa-ai hooks, or no observations will be captured."
...
**Independent Test**: Install the plugin into a temp HOME, start the tools-api (`bun run dev:api`), run a Codex session or simulate a hook event via `echo '{}' | bun apps/claude-plugin/hooks/massa-ai-hook.ts session-start`, and confirm an Observation row appears in PG.

---

### P2: Cursor plugin bundle

**User Story**: As a Cursor user, I want a `massa-ai` plugin directory I can register via `vscode.cursor.plugins.registerPath` or copy into `~/.cursor/`, so that I get skills, MCP tools, and the full lifecycle hook coverage (including `sessionStart` and `preCompact`, which were previously missing).
...
1. WHEN a user runs `apps/cursor-plugin/install.sh --user` THEN the system SHALL create `~/.cursor/plugins/massa-ai/` containing `skills/*.md`, `hooks/hooks.json`, `mcp.json`, `commands/*.md`, and `agents/massa-ai-navigator.md`, with an ownership marker
2. WHEN a user runs `apps/cursor-plugin/install.sh --project` THEN the system SHALL create `./.cursor/plugins/massa-ai/` under the current project
...
4. WHEN the Cursor plugin is installed AND `mcp.json` is present THEN Cursor SHALL auto-discover the massa-ai MCP server without a separate `install-agents.ts` run for that scope. The installer SHALL print a hint: "If you also run `install-agents.ts --agent cursor`, skip MCP — the plugin already registers it."
...
**User Story**: As a user setting up massa-ai, I want the root `install.sh` and `install-agents.ts` to offer Codex/Cursor plugin installation, and the README to document it, so that I don't need to find the per-plugin installers manually.
...
- WHEN `MASSA_AI_PROJECT_ID` is unset AND no pin file exists AND `git rev-parse` fails (not a git repo) THEN the binary SHALL fall back to `cwd` basename and still POST
...
- WHEN a user has a manual `massa-ai` block in `~/.codex/hooks.json` or `~/.cursor/hooks.json` (no ownership marker) THEN the installer SHALL back up the file (`.massa-ai.bak-<ts>`) and merge without clobbering the user's keys
...
- WHEN stdin is a terminal (char device) THEN the binary SHALL NOT attempt to read stdin and SHALL exit 0 with no POST (matches `massa-ai-hook.ts:115-125`)
...
**User Story**: As a user setting up massa-ai, I want the root `install.sh` plugin menu to offer ALL four supported coding tools (Claude Code, Codex, Cursor, OpenCode) in one place, and I want passive-capture hooks auto-written for whichever I choose — not just Codex and Cursor — so that every tool I use gets the same skills + hooks + MCP bundle experience.
...
3. WHEN the user selects OpenCode from the plugin menu THEN the system SHALL either invoke the existing npm plugin install (`npm install @massa-ai/opencode-plugin` + config snippet) OR, if from source, build and link `apps/opencode-plugin/`; AND print the OpenCode hook config (OpenCode hooks are in-process via the Plugin API, not subprocess hooks.json, so no external hooks file is auto-written)
4. WHEN Claude Code hooks are auto-written THEN the 5 Claude events (SessionStart, UserPromptSubmit, PostToolUse, PreCompact, Stop) SHALL be wired to the shared `massa-ai-hook.ts` binary via `settings.json` merge (NOT the old `settings.json.template` print-only approach), using the same `_massaAiOwned` marker + backup pattern as Codex/Cursor
...
6. WHEN `install-agents.ts --agent claude-code` runs THEN it SHALL print a deconfliction hint: "If you installed the massa-ai Claude plugin, hooks are already wired — skip this"
...
- [ ] Both plugins reuse the `massa-ai-hook.ts` binary for fire-and-forget POSTs with pin resolution and pre-compact dual-POST
