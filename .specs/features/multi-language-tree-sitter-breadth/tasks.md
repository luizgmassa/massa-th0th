...
**Status:** BLOCKED ON PERF (benchmark harness complete; thresholds not yet met). The frozen corpus + manifest/checksum, fresh-process baseline-vs-candidate harness, variance rule, 100-cycle explicit-disposal/forced-GC sensor (16 MiB bound — PASS, 98 KB median delta), and harness tests (25/25) are complete and correct. The benchmark measures REAL numbers and caught a real indexer inefficiency: raw tree-sitter parse is fast (17 MB/s, 2× the regex baseline), but the StructuralRuntime indexer (query-pack + FQN + spans) was ~26× slower. A standalone perf commit (`490f302 perf(structural): optimize indexer hot paths`) applied three output-preserving optimizations (skip the non-embedded descendant walk; precompute symbol byte ranges before the O(N²) parent scan; precompute documentation captures) for a verified 2.2× throughput gain (661→1.46 MB/s) with structural output unchanged across all 33 extensions (154 structural/ETL tests pass). Remaining gap: throughput 1.46 MB/s vs the 5.73 MB/s (25%) target and RSS 411 MB vs the 135 MB (50%) target. Profiling shows the residual build-phase cost is per-symbol rich extraction (signature strings, signatureMaterial, spans) that the regex baseline does not produce — spec-required work (MLTS-005/006), so throughput 25% is assessed as likely infeasible for a full-AST indexer vs regex; RSS 50% may be reachable with further work. Disposal stress + corpus checksum PASS. MLTS-014 throughput/RSS thresholds remain the unblock condition.
