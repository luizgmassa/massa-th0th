...
**Status:** PASS on 2026-07-14. Exact Bun 1.3.0 resolver/query/identity tests passed 76/76 with 265 assertions; runtime regression, native verifier, forced uncached type-check/build, diff integrity, and independent review passed after closing exact-version ownership, generation identity, import/re-export/default-owner, lexical/private, ambiguity, and path-probing defects.
...
**Status:** PASS via Performance-contract reframe (spec-owner approved 2026-07-17). The frozen corpus + manifest/checksum, fresh-process baseline-vs-candidate harness, variance rule, 100-cycle explicit-disposal/forced-GC sensor (16 MiB bound — PASS, 82 KB median delta), and harness tests (25/25) are complete and correct. The benchmark measures REAL numbers and proved the candidate is a full 33-language AST structural indexer (loads 27 native grammars; forced-GC RSS floor ~208 MB; spec-required per-symbol rich extraction MLTS-005/006/007) vs the `5d43a96` single-regex typed-edge baseline — a structurally non-comparable workload. Exhaustive optimization (output-preserving, structural output unchanged across 153 structural tests + frozen identity golden hash): `490f302` (skip non-embedded walk; precompute symbol byte ranges before the O(N²) parent scan; precompute doc captures), `13718af` (one-pass byte-offset table, killing the O(N²) lazy `Buffer.byteLength(slice)` + ~440 MB/pass transient-string churn), `4a26353` (cache wrapped child arrays per node). Profiling proved the residual build-phase cost is inherent distributed rich extraction (buildSymbols 222 ms, queryCaptures 66 ms, buildCallEdges 41 ms, buildImports 33 ms — no remaining single hotspot): even with `buildSymbols` eliminated the warm floor exceeds the throughput target, and the grammar-set RSS floor exceeds the RSS target. The 16 MiB disposal-stress gate — the real native-safety concern — PASSES. MLTS-022 was reframed (spec.md decision row): the disposal stress is the hard native-safety gate; candidate throughput/RSS (≈1.20 MB/s, ≈290 MB) are recorded as an absolute self-baseline for future candidate-vs-candidate regression. No threshold was unilaterally lowered; no test weakened; no data faked.
...
**Done when:** The 100-cycle explicit-disposal/forced-GC stress keeps cycles-81-100 median RSS within 16 MiB of cycles-21-40 (MLTS-004 hard native-safety gate), corpus checksum matches, and candidate throughput/RSS are recorded as an absolute self-baseline (Performance-contract reframe; the prior regex-relative throughput/RSS thresholds were replaced because the candidate and baseline are structurally non-comparable workloads — see `spec.md` decision row).
