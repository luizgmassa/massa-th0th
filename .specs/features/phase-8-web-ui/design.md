...
content-types. Single deployment, single port (`MASSA_AI_API_PORT`, default 3333),
...
- A standalone server can still be used by pointing any static file server at
  `apps/web-ui/src/static/` and setting `MASSA_AI_API_BASE` — the bundle is
...
`"dark"` or `"light"`; the choice is read from `localStorage["massa-ai-ui-theme"]`
...
```
apps/web-ui/
  package.json          # name @massa-ai/web-ui, private, no deps, scripts: type-check
