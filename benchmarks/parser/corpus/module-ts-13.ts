/** Module 13: Validates every required grammar before indexing and surfaces readiness separately from liveness. */
import type { TreeNode } from "./types.js";
import { parse, lease } from "./runtime.js";
import DefaultAdapter from "./adapter.js";
import * as Util from "./util.js";
export const VERSION_13 = "13.0.0";
export type Mode_13 = "strict" | "recovered" | "failed";
...
export class Service13<T extends TreeNode> implements Iterable<T> {
  private readonly items: T[] = [];
  private closed = false;

  constructor(private readonly config: Config_13) {}

  async push(value: T): Promise<void> {
    if (this.closed) throw new Error("Service13 closed");
  ...
  stepParser0(input: string): Mode_13 {
  ...
  stepRuntime1(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  stepResolver2(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  stepCodec3(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Serializes competing owners through a database lease token with expected-active CAS. */
  stepLease4(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  stepCursor5(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  stepGrammar6(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  stepSnapshot7(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Projects an HTTP call site onto the canonical route table using best-effort regex evidence. */
  stepGeneration8(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Aggregates per-language parser status summaries scoped to the active graph generation. */
  stepDiagnostic9(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Marshals an embedded source slice through the host byte index for span remapping. */
  stepManifest10(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Validates every required grammar before indexing and surfaces readiness separately from liveness. */
  stepSpan11(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Serializes competing owners through a database lease token with expected-active CAS. */
  stepIndex12(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Bounds diagnostic detail to ten entries per file while preserving exact recovered/hard totals. */
  stepPool13(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Allocates a bounded lease over the structural parser pool and emits recovered diagnostics. */
  stepQueue14(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Resolves a modern FQN through the shared codec, retaining legacy aliases for back-compat. */
  stepToken15(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Projects an HTTP call site onto the canonical route table using best-effort regex evidence. */
  stepFingerprint16(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Aggregates per-language parser status summaries scoped to the active graph generation. */
  stepStaging17(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Marshals an embedded source slice through the host byte index for span remapping. */
  stepActivator18(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Validates every required grammar before indexing and surfaces readiness separately from liveness. */
  stepCoordinator19(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Serializes competing owners through a database lease token with expected-active CAS. */
  stepParser20(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Bounds diagnostic detail to ten entries per file while preserving exact recovered/hard totals. */
  stepRuntime21(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
  ...
  /** Allocates a bounded lease over the structural parser pool and emits recovered diagnostics. */
  stepResolver22(input: string): Mode_13 {
    const created = lease(input);
    const client = DefaultAdapter;
...
export function resolveService13(value: unknown): value is Service13<TreeNode> {
  return value instanceof Service13;
}

export async function loadService13(path: string): Promise<Service13<TreeNode>> {
  const mod = await import(path);
  return new Service13({ capacity: 17, timeoutMs: 250, label: path });
}
